...
--                         ELSE ROUND(standard_amt_usd/standard_qty , 2)
-- 					END AS unit_price
-- FROM orders
-- LIMIT 10;
...
-- Q 1 : Write a query to display for each order, the account ID, total amount of the order, and the level of the order 
...
-- Q 2 : Write a query to display the number of orders in each of three categories, 
...
-- 			ELSE 'Less than 1000'
-- 		END AS total_num_items_in_each_order,
-- 	COUNT(*) AS order_count
-- FROM orders
-- GROUP BY 1
-- ORDER BY 2 DESC

-- Q 3 : We would like to understand 3 different levels of customers based on the amount associated 
...
-- SELECT a.name, SUM(total_amt_usd) life_time_value,
...
-- Q 4 : We would now like to perform a similar calculation to the first, but we want to obtain the 
...
-- FROM orders o JOIN accounts a
-- 	ON o.account_id = a.id
-- WHERE occurred_at > '2015-12-31' -- WHERE o.occurred_at BETWEEN '2016-01-01' AND '2017-12-31'
-- GROUP BY 1
-- ORDER BY 2 DESC

-- Q 5 : We would like to identify top performing sales reps, which are sales reps associated with more than 200 orders. 
...
-- SELECT s.name, count(o.id) total_number_orders, CASE WHEN count(o.id) >= 200 THEN 'top'												
-- 													ELSE 'not'
...
-- Q 6 : The previous didn't account for the middle, nor the dollar amount associated with the sales. 
